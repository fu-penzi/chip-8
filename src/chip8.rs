...
impl Chip8 {
    ...
    fn op_00e0(&mut self) {
        ...
        // for col in self.video.iter_mut() {
        //     *col = 0;
        // }
    }
    ...
    // Pop address from stack and set PC as popped address
    fn op_00ee(&mut self) {
        self.sp -= 1;
    ...
    // CALL NNN
    // Put current PC on stack; set PC to NNN
    fn op_2nnn(&mut self) {
    ...
    // SHR Vx, Vy
    // Shift right by 1, !!!! Ambiguous instruction!
    fn op_8xy6(&mut self) {
        let x: usize = ((self.opcode & 0x0F00) >> 8) as usize;
    ...
    // DRW Vx, Vy, nibble
    // Draw N bytes sized sprite from RAM[I] to display at [Vx][Vy].
    fn op_dxyn(&mut self) {
        let x: usize = ((self.opcode & 0x0F00) >> 8) as usize;
        let y: usize = ((self.opcode & 0x00F0) >> 4) as usize;
    ...
    fn op_fx33(&mut self) {
        ...
        let digit_3: u8 = v_x % 10;
        let digit_2: u8 = (v_x % 100 - digit_3) / 10;
        let digit_1: u8 = (v_x - (digit_2 * 10) - digit_3) / 100;

        self.ram[self.i as usize] = digit_1;
        self.ram[(self.i + 1) as usize] = digit_2;
        self.ram[(self.i + 2) as usize] = digit_3;
    }
